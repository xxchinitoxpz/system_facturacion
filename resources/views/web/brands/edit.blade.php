<x-app-layout>
    <div class="max-w-full mx-auto bg-white rounded-xl shadow p-4 mt-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-indigo-800">Editar Marca: {{ $brand->nombre }}</h1>
        ...
        <form id="editBrandForm" action="{{ route('brands.update', $brand) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            
            <!-- Información de la Marca -->
            <div class="bg-indigo-50 rounded-lg p-6">
                <h3 class="text-lg font-semibold text-indigo-800 mb-4">Información de la Marca</h3>
                <div>
                    ...
                    <input type="text" id="nombre" name="nombre" 
                           ...
                           placeholder="Ej: Samsung, Apple, Nike, etc."
                           required>
                    @error('nombre')
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200 justify-end">
                <a href="{{ route('brands.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
                    Cancelar
                </a>
                <button type="submit" id="submitBtn" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2">
                    <span id="submitText">Actualizar Marca</span>
