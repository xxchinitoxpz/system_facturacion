<x-app-layout>
    <div class="max-w-full mx-auto bg-white rounded-xl shadow p-4 mt-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-indigo-800">Detalles de la Marca</h1>
            <a href="{{ route('brands.index') }}" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors">
                Volver
            </a>
        </div>
        ...
        <div class="bg-indigo-50 rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900">{{ $brand->nombre }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Fecha de Creación</label>
                    <p class="text-sm text-gray-900">{{ $brand->created_at->format('d/m/Y H:i:s') }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Última Actualización</label>
                    <p class="text-sm text-gray-900">{{ $brand->updated_at->format('d/m/Y H:i:s') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-3 pt-6 border-t border-gray-200 justify-end">
            @can('editar-marcas')
                <a href="{{ route('brands.edit', $brand) }}" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                    Editar Marca
                </a>
            @endcan
            @can('eliminar-marcas')
    ...
    <form id="deleteForm" action="{{ route('brands.destroy', $brand) }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
    </form>
    ...
    <script>
        function confirmDelete() {
            if (confirm('¿Estás seguro de que quieres eliminar la marca "{{ $brand->nombre }}"? Esta acción no se puede deshacer.')) {
