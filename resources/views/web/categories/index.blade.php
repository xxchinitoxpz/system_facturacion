<x-app-layout>
    <div class="max-w-full mx-auto bg-white rounded-xl shadow p-4 mt-8">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-indigo-800">Categorías</h1>
            @can('crear-categorias')
                <a href="{{ route('categories.create') }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors">
                    Crear Categoría
                </a>
            @endcan
        </div>
        ...
        <div class="mb-6">
            <form method="GET" action="{{ route('categories.index') }}" class="flex gap-4">
                ...
                @if($search)
                    <a href="{{ route('categories.index') }}" class="px-6 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors">
                        Limpiar
                    </a>
                @endif
            </form>
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead class="bg-indigo-100">
                    <tr>
                        ...
                        @if(auth()->user()->canAny(['ver-categorias', 'editar-categorias', 'eliminar-categorias']))
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($categories as $category)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-sm text-gray-900">{{ $category->nombre }}</td>
                            ...
                            <td class="px-4 py-2 text-sm text-gray-500">{{ $category->created_at->format('d/m/Y H:i') }}</td>
                            @if(auth()->user()->canAny(['ver-categorias', 'editar-categorias', 'eliminar-categorias']))
                                <td class="px-4 py-2 text-sm text-gray-500">
                                        ...
                                        @can('eliminar-categorias')
                                            @if($category->products_count > 0)
                                                <button disabled
                                                        ...
                                                        title="No se puede eliminar porque tiene {{ $category->products_count }} producto(s) asociado(s)">
                                                    Eliminar
                                                </button>
                                            @else
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ auth()->user()->canAny(['ver-categorias', 'editar-categorias', 'eliminar-categorias']) ? 5 : 4 }}" class="px-4 py-8 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    @can('crear-categorias')
                                        <a href="{{ route('categories.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                ...
                <div class="p-6">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-500 mb-6">
                            ¿Estás seguro de que quieres eliminar la categoría "<span id="categoryName" class="font-semibold"></span>"?
                            <br>
