<x-app-layout>
    <div class="max-w-full mx-auto bg-white rounded-xl shadow p-4 mt-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-indigo-800">Detalles del Cliente</h1>
            <div class="flex gap-2">
                @can('editar-clientes')
                    <a href="{{ route('clients.edit', $client->id) }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
                        Editar
                    </a>
                @endcan
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="mt-1 text-lg font-semibold text-gray-900">{{ $client->nombre_completo }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500">Tipo de Documento</label>
                            <p class="mt-1 text-lg font-semibold text-gray-900">{{ $client->tipo_documento }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500">Número de Documento</label>
                            <p class="mt-1 text-lg font-semibold text-gray-900">{{ $client->nro_documento }}</p>
                        </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="mt-1 text-lg font-semibold text-gray-900">
                                @if($client->telefono)
                                    <a href="tel:{{ $client->telefono }}" class="text-indigo-600 hover:text-indigo-800">
                                        {{ $client->telefono }}
                                    </a>
                                @else
            ...
            <div class="mt-8 pt-6 border-t border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="mt-1 text-lg font-semibold text-gray-900">{{ $client->created_at->format('d/m/Y H:i') }}</p>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-500">Última Actualización</label>
                        <p class="mt-1 text-lg font-semibold text-gray-900">{{ $client->updated_at->format('d/m/Y H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Acciones -->
        @if(auth()->user()->canAny(['editar-clientes', 'eliminar-clientes']))
            <div class="mt-6 pt-6 border-t border-gray-200">
                ...
                <div class="flex gap-3">
                    ...
                    @can('eliminar-clientes')
                        <form action="{{ route('clients.destroy', $client->id) }}" method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este cliente? Esta acción no se puede deshacer.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
