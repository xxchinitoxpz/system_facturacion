<x-app-layout>
    <div class="max-w-full mx-auto bg-white rounded-xl shadow p-4 mt-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-indigo-800">Editar Combo</h1>
        ...
        <form id="editComboForm" method="POST" action="{{ route('combos.update', $combo) }}" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="mt-4">
                    <label class="flex items-center">
                        <input 
                            ...
                            name="estado" 
                            value="1"
                            {{ old('estado', $combo->estado) ? 'checked' : '' }}
                                     ...
                                     >
                                         ...
                                         @foreach($products as $productOption)
                                             <option value="{{ $productOption->id }}" 
                                                 {{ $product->id == $productOption->id ? 'selected' : '' }}
                                                 data-precio="{{ $productOption->presentations->first()->precio_venta ?? 0 }}">
                                                 {{ $productOption->nombre }} - {{ $productOption->category->nombre }} - {{ $productOption->brand->nombre }}
                                             </option>
                                         @endforeach
                                     </select>
                                 </div>
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button type="submit" id="submitBtn" class="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors flex items-center gap-2">
                    <span id="submitText">Actualizar Combo</span>
    ...
    <template id="productTemplate">
        <div class="producto-item border border-gray-200 rounded-lg p-4 bg-white">
         ...
         document.addEventListener('DOMContentLoaded', function() {
            ...
            // Agregar event listeners para botones de eliminar existentes
            ...
            function addProduct() {
             ...
             document.getElementById('editComboForm').addEventListener('submit', function(e) {
