    ...
    <div class="max-w-full mx-auto bg-white rounded-xl shadow p-4 mt-8">
        ...
        <form id="editRoleForm" action="{{ route('roles.update', $role->id) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-indigo-50 rounded-lg p-6">
                ...
                <!-- Reportes -->
                <div class="mb-6">
                    <h4 class="text-md font-semibold text-indigo-700 mb-3 border-b border-indigo-200 pb-2">Reportes</h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <label class="flex items-center">
                            <input type="checkbox" name="permissions[]" value="ver-reportes"
                                   {{ $role->permissions->contains('name', 'ver-reportes') ? 'checked' : '' }}
                                   class="w-4 h-4 text-indigo-600 bg-gray-100 border-gray-300 rounded focus:ring-indigo-500 focus:ring-2">
                            <span class="ml-2 text-sm font-medium text-gray-700">Ver Reportes</span>
                        </label>
                    </div>
                </div>
