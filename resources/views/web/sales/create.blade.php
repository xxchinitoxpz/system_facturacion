<x-app-layout>
    <div class="max-w-full mx-auto bg-white rounded-xl shadow p-4 mt-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-indigo-800">Crear Venta</h1>
        ...
        @endif
                ...
                <form id="createSaleForm" method="POST" action="{{ route('sales.store') }}" class="space-y-6">
            @csrf
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                     <div>
                         ...
                         >
                             ...
                             <option value="ticket" {{ old('tipo_comprobante', 'ticket') == 'ticket' ? 'selected' : '' }} selected>Ticket</option>
                         </select>
                     ...
                     <div>
                         ...
                         <input type="hidden" id="cliente_id" name="cliente_id" value="{{ old('cliente_id', '1') }}">
                     ...
                     <div>
                         ...
                         <input 
                             ...
                             id="cliente_nombre" 
                             name="cliente_nombre"
                             value="{{ old('cliente_nombre', 'CLIENTE GENERAL') }}"
                         ...
                         <div>
                             ...
                             >
                                 ...
                                 @foreach($branches as $branch)
                                     <option value="{{ $branch->id }}" {{ old('sucursal_id', $openCashBox->sucursal_id) == $branch->id ? 'selected' : '' }}>
                                         {{ $branch->nombre }}
                                     </option>
                                 @endforeach
                             </select>
             ...
             <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                 ...
                 <div class="space-y-6">
                     ...
                     <div class="bg-gray-50 p-4 rounded-lg">
                         ...
                         <div id="pago_unico" class="space-y-4">
                             <div>
                         ...
                         <div id="pago_dividido" class="space-y-6 hidden">
                             <!-- Pago 1 -->
                             ...
                             <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                 <div class="flex justify-between items-center">
                                     ...
                                     <span class="text-lg font-bold text-blue-800" id="total_venta_display">S/ 0.00</span>
                                 </div>
                                 <div class="flex justify-between items-center mt-2">
                                     <span class="text-sm font-medium text-blue-700">Suma de Pagos:</span>
                                     <span class="text-lg font-bold" id="suma_pagos_display">S/ 0.00</span>
                                 </div>
                                 <div class="flex justify-between items-center mt-2">
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200 justify-end">
                ...
                <button type="submit" id="submitBtn" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2">
                    <span id="submitText">Crear Venta</span>
                         ...
                         clienteSpinner.classList.add('hidden');
                              ...
                              if (data.data.cliente_existente) {
                                  ...
                                  if (tipoComprobante === 'factura') {
                                      // Para facturas, crear el cliente automáticamente
                                      crearClienteEnBD(documento, nombre, data.tipo);
                                  } else {
                                      // Para boletas y tickets, usar temporal
          ...
          function mostrarPresentaciones(presentaciones, stockDisponible, precioUnitario) {
           ...
           function agregarPresentacion(presentacionId, nombrePresentacion, precioVenta, unidades, maximoPresentaciones) {
               ...
               // Limpiar campos de búsqueda y presentaciones
               limpiarBusquedaProducto();
          ...
          function actualizarTotal() {
              ...
              // Si el pago dividido está activo, actualizar también su display
              const dividirPago = document.getElementById('dividir_pago').checked;
              if (dividirPago) {
                  console.log('Pago dividido activo, actualizando display...');
                  actualizarDisplayDivisionPago();
              }
          }
           ...
           function toggleDivisionPago() {
               ...
               if (dividirPago) {
                   pagoUnico.classList.add('hidden');
                   pagoDividido.classList.remove('hidden');
                   ...
                   pagoUnico.classList.remove('hidden');
                   pagoDividido.classList.add('hidden');
           ...
           function actualizarDisplayDivisionPago() {
               ...
               console.log('Actualizando display división pagos. Total venta:', totalVenta, 'Suma pagos:', sumaPagos, 'Diferencia:', diferencia);
               ...
               console.log('Display actualizado. Total venta display:', document.getElementById('total_venta_display').textContent, 'Suma pagos display:', document.getElementById('suma_pagos_display').textContent);
           }
           ...
           function limpiarCamposPagoDividido() {
               ...
               // Resetear display solo cuando se desactiva el pago dividido
          ...
          document.getElementById('createSaleForm').addEventListener('submit', function(e) {
              ...
              if (dividirPago) {
                  // Log para depuración del pago dividido
                  console.log('Validando pago dividido...');
                  console.log('Tipo pago 1:', document.getElementById('tipo_pago_1').value);
                  console.log('Tipo pago 2:', document.getElementById('tipo_pago_2').value);
                  console.log('Monto pago 1:', document.getElementById('monto_pago_1').value);
                  console.log('Monto pago 2:', document.getElementById('monto_pago_2').value);
              ...
              this.submit();
                     ...
                     // Cargar cliente general automáticamente al cargar la página
           document.addEventListener('DOMContentLoaded', function() {
               const tipoComprobante = document.getElementById('tipo_comprobante').value;
               if (tipoComprobante === 'ticket') {
                   // Buscar el cliente general en la lista de clientes
                   const clienteTicket = clients.find(client => client.nro_documento === '00000000');
                   if (clienteTicket) {
                       const clienteDocumentoInput = document.getElementById('cliente_documento');
                       const clienteIdInput = document.getElementById('cliente_id');
                       ...
                       clienteDocumentoInput.value = clienteTicket.nro_documento;
                       clienteIdInput.value = clienteTicket.id;
                       clienteNombreInput.value = clienteTicket.nombre_completo;
                       clienteDocumentoInput.readOnly = true;
                       clienteDocumentoInput.placeholder = 'Cliente automático (Ticket)';
               ...
               // Ocultar campos de monto recibido y vuelto por defecto
               document.getElementById('monto_recibido_container').classList.add('hidden');
               document.getElementById('vuelto_container').classList.add('hidden');
           ...
           function mostrarMensajeError(mensaje) {
               ...
               mensajeDiv.className = 'fixed top-4 right-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded z-50 max-w-md';
               mensajeDiv.innerHTML = `
                   <div class="flex items-start">
                       ...
                       <div class="flex-1">
                           <div class="whitespace-pre-line text-sm">${mensaje}</div>
                       </div>
           ...
           // Función para crear cliente en la base de datos
           function crearClienteEnBD(documento, nombre, tipo) {
               const clienteData = {
               ...
               .then(data => {
                   if (data.success) {
                       ...
                       mostrarMensajeExito(`Cliente creado exitosamente: ${nombre}`);
                       
                       // Actualizar la lista de clientes
                       clients.push(data.cliente);
                   } else {
                       // Error al crear cliente
                       mostrarMensajeError('Error al crear cliente: ' + (data.message || 'Error desconocido'));
                       // Usar cliente temporal como fallback
                       document.getElementById('cliente_id').value = 'temp_' + documento;
                   }
               })
               .catch(error => {
                   console.error('Error al crear cliente:', error);
                   mostrarMensajeError('Error al crear cliente. Usando cliente temporal.');
                   // Usar cliente temporal como fallback
                   document.getElementById('cliente_id').value = 'temp_' + documento;
               });
           }
           
           // Función para limpiar búsqueda de producto
           ...
           function mostrarMensajeExito(mensaje) {
               // Crear elemento de mensaje
               const mensajeDiv = document.createElement('div');
               mensajeDiv.className = 'fixed top-4 right-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded z-50 max-w-md';
               mensajeDiv.innerHTML = `
                   <div class="flex items-start">
                       <svg class="w-5 h-5 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
               ...
               // Remover automáticamente después de 8 segundos (más tiempo para mensajes largos)
               setTimeout(() => {
                   if (mensajeDiv.parentElement) {
                       mensajeDiv.remove();
                   }
