<x-app-layout>
    <div class="max-w-full mx-auto bg-white rounded-xl shadow p-4 mt-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-indigo-800">Editar Almacén: {{ $warehouse->nombre }}</h1>
        ...
        <form id="editWarehouseForm" action="{{ route('warehouses.update', $warehouse) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            
            <!-- Información del Almacén -->
            <div class="bg-indigo-50 rounded-lg p-6">
                <h3 class="text-lg font-semibold text-indigo-800 mb-4">Información del Almacén</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input 
                            ...
                            placeholder="Ej: Almacén Principal, Bodega Norte, etc."
                            required
                        >
                        @error('nombre')
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($branches as $branch)
                                <option value="{{ $branch->id }}" {{ old('sucursal_id', $warehouse->sucursal_id) == $branch->id ? 'selected' : '' }}>
                                    {{ $branch->nombre }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-6">
                    ...
                    >{{ old('descripcion', $warehouse->descripcion) }}</textarea>
                    @error('descripcion')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200 justify-end">
                <a href="{{ route('warehouses.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
                    Cancelar
                </a>
                <button type="submit" id="submitBtn" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2">
                    <span id="submitText">Actualizar Almacén</span>
